...
def b1_f():
    ...
    now = time.localtime()
    data = {'loc': user_locate, 'time': "%04d/%02d/%02d %02d:%02d:%02d"%(now.tm_year, now.tm_mon, now.tm_mday, now.tm_hour, now.tm_min, now.tm_sec),'user_type': 'barrier_free', 'user_id': user_id}
    qr_res = requests.post('http://161.122.53.222:8080/qr_receive', data = json.dumps(data))

    qr_img = './qr_img/B1_F.jpg'
    return render_template('web_B1_F.html', user_loc = user_locate, fire_loc = fire_locate, loc_img = qr_img)
...
def f1_a():
    ...
    return render_template('web_1F_A.html', user_loc = user_locate, fire_loc = fire_locate, loc_img = qr_img)
...
def f3_a():
    ...
    return render_template('web_3F_A.html', user_loc = user_locate, fire_loc = fire_locate, loc_img = qr_img)

@app.route('/end', methods=['GET', 'POST'])
def situ_end():
    data = {'user_id': user_id}
    end_res = requests.post('http://161.122.53.222:8080/end_receive', data = json.dumps(data))

    return render_template('web_index.html')

if __name__ == '__main__':
    app.run(port=9070, threaded=False)